...
namespace ItemManager
{
    ...
    public class RequiredResourceList
    {
        ...
        public void Add(string itemName, ConfigEntry<int> amountConfig) => Requirements.Add(new Requirement
            { itemName = itemName, amountConfig = amountConfig });
    }
    ...
    public class Item
    {
        ...
        public Item(AssetBundle bundle, string prefabName) : this(
            PrefabManager.RegisterPrefab(bundle, prefabName, true), true)
        {
        }
        ...
        public void ToggleConfigurationVisibility(Configurability visible)
        {
            void Toggle(ConfigEntryBase cfg, Configurability check)
            {
                foreach (object? tag in cfg.Description.Tags)
                {
                    ...
                    {
                        attrs.Browsable = (visible & check) != 0 &&
                                          (attrs.browsability is null || attrs.browsability());
                    }
                }
            }
                ...
                foreach (Item item in registeredItems.Where(i => i.configurability != Configurability.Disabled))
                {
                    ...
                    if ((item.configurability & Configurability.Recipe) != 0)
                    {
                        ...
                        foreach (string configKey in item.Recipes.Keys.DefaultIfEmpty(""))
                        {
                            ...
                            if (item.Recipes.ContainsKey(configKey) &&
                                item.Recipes[configKey].Crafting.Stations.Count > 0)
                            {
                                ...
                                {
                                    if (activeRecipes.ContainsKey(item) && activeRecipes[item]
                                ...
                                cfg.tableLevel = config(englishName, "Crafting Station Level" + configSuffix,
                                    ...
                                    new ConfigDescription($"Required crafting station level to craft {englishName}.",
                                        null, tableLevelAttributes));
                                ...
                                if (item.Prefab.GetComponent<ItemDrop>().m_itemData.m_shared.m_maxQuality > 1)
                                {
                                    cfg.maximumTableLevel = config(englishName,
                                        ...
                                        new ConfigDescription(
                                            $"Maximum crafting station level to upgrade and repair {englishName}.",
                                            null, tableLevelAttributes));
                                }

                                bool QualityResultBrowsability() => cfg.requireOneIngredient.Value == Toggle.On;
                                cfg.requireOneIngredient = config(englishName,
                                    "Require only one resource" + configSuffix,
                                ...
                                cfg.qualityResultAmountMultiplier = config(englishName,
                                    "Quality Multiplier" + configSuffix,
                                ...
                                ConfigEntry<string> itemConfig(string name, string value, string desc)
                                {
                                    ...
                                    return config(englishName, name, value,
                                        ...
                                        .All(r => r.amountConfig is null))
                                {
                                    cfg.upgrade = itemConfig("Upgrading Costs" + configSuffix,
                                        new SerializedRequirements(item.Recipes[configKey].RequiredUpgradeItems
                        ...
                        for (int i = 0; i < item.Conversions.Count; ++i)
                        {
                            ...
                            void UpdatePiece()
                            {
                                ...
                                if (conversion.config.activePiece is not null)
                                {
                                    ...
                                    if (removeIndex >= 0)
                                    {
                                        foreach (Smelter instantiatedSmelter in
                    ...
                    if ((item.configurability & Configurability.Stats) != 0)
                    {
                            ...
                            Action<ItemDrop.ItemData.SharedData, T> setValue)
                        {
                            ItemDrop.ItemData.SharedData shared =
                                item.Prefab.GetComponent<ItemDrop>().m_itemData.m_shared;
                            ...
                            void ApplyConfig()
                            {
                                ...
                                Inventory[] inventories = Player.s_players.Select(p => p.GetInventory())
                                    .Concat(UnityEngine.Object.FindObjectsOfType<Container>()
                                        .Select(c => c.GetInventory())).Where(c => c is not null).ToArray();
                            ...
                            or ItemDrop.ItemData.ItemType.TwoHandedWeaponLeft)
                        {
                            ...
                            statcfg("Movement Speed Modifier", $"Movement speed modifier of {englishName}.",
                                shared => shared.m_movementModifier,
                                (shared, value) => shared.m_movementModifier = value);
                        }
                        ...
                        if (itemType is ItemDrop.ItemData.ItemType.Consumable && shared.m_food > 0)
                        {
                            ...
                            statcfg("Health Regen", $"Health regen value of {englishName}.",
                                shared => shared.m_foodRegen, (shared, value) => shared.m_foodRegen = value);
                        }
                            ...
                            or ItemDrop.ItemData.ItemType.TwoHandedWeaponLeft or ItemDrop.ItemData.ItemType.Bow)
                        {
                            ...
                            statcfg("Backstab Bonus", $"Backstab bonus of {englishName}.",
                                shared => shared.m_backstabBonus, (shared, value) => shared.m_backstabBonus = value);
                            ...
                            SetDmg("Chop", dmg => dmg.m_chop,
                                (ref HitData.DamageTypes dmg, float val) => dmg.m_chop = val);
                            SetDmg("Pickaxe", dmg => dmg.m_pickaxe,
                                (ref HitData.DamageTypes dmg, float val) => dmg.m_pickaxe = val);
                            SetDmg("Fire", dmg => dmg.m_fire,
                                (ref HitData.DamageTypes dmg, float val) => dmg.m_fire = val);
                            ...
                            if (itemType is ItemDrop.ItemData.ItemType.Bow)
                            {
                                ...
                                statcfg("Maximum Draw Time",
                                    $"Time until {englishName} is fully drawn at skill level 0.",
                    ...
                    foreach (CraftingStationConfig station in kv.Value.Crafting.Stations)
                    {
                        ...
                        recipe.m_resources = SerializedRequirements.toPieceReqs(__instance,
                            ...
                            cfg?.upgrade == null
                                ...
                                : new SerializedRequirements(cfg.upgrade.Value));
                        if ((cfg == null || recipes.Count > 0 ? station.Table : cfg.table.Value) is CraftingTable
                                .Inventory or CraftingTable.Disabled)
                        {
                        ...
                        recipe.m_minStationLevel =
                            cfg == null || recipes.Count > 0 ? station.level : cfg.tableLevel.Value;
        ...
        public void AssignDropToCreature()
        {
            ...
            foreach (KeyValuePair<Character, CharacterDrop.Drop> kv in drops.toCharacterDrops(ZNetScene.s_instance,
                         Prefab))
            {
        ...
        private static void drawDropsConfigTable(ConfigEntryBase cfg)
        {
                         ...
                         { min = 1, max = 1, creature = "", chance = 1 }))
            {
                ...
                int min = drop.min;
                if (int.TryParse(
                        GUILayout.TextField(min.ToString(), new GUIStyle(GUI.skin.textField) { fixedWidth = 35 }),
                ...
                int max = drop.max;
                if (int.TryParse(
                        GUILayout.TextField(max.ToString(), new GUIStyle(GUI.skin.textField) { fixedWidth = 35 }),
                ...
                if (wasAdded && !locked)
                {
                    wasUpdated = true;
                    newDrops.Add(new DropTarget
                        { min = 1, max = 1, creature = "", chance = 1, levelMultiplier = true });
                }
            }
    ...
    public static class PrefabManager
    {
        static PrefabManager()
        {
            ...
            harmony.Patch(AccessTools.DeclaredMethod(typeof(ObjectDB), nameof(ObjectDB.CopyOtherDB)),
                postfix: new HarmonyMethod(
                    AccessTools.DeclaredMethod(typeof(PrefabManager), nameof(Patch_ObjectDBInit))));
            ...
            harmony.Patch(AccessTools.DeclaredMethod(typeof(Smelter), nameof(Smelter.OnAddFuel)),
                postfix: new HarmonyMethod(AccessTools.DeclaredMethod(typeof(Item),
                    nameof(Item.Patch_OnAddSmelterInput))));
        ...
        {
            Debug.Log("Patch_ZNetSceneAwake 1");
            foreach (GameObject prefab in prefabs.Concat(ZnetOnlyPrefabs))
            {
                Debug.Log($"Patch_ZNetSceneAwake 1 {prefab.name}");
